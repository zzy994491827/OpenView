...
data_dict = np.load('./dataset/omniobject3D/point_clouds_with_image_text_sampled_for_retrieval.npy', allow_pickle=True).item()
with torch.no_grad():
    for key, value in tqdm(data_dict.items()):
        ...
        text_features_list.append(torch.squeeze(torch.from_numpy(data_dict[key]["text_feat_1280"]).type(torch.float32).to(device)))
        ...
        three_d_features = model(images_feat,N_sample,xyz, feat,None,False,None)
    ...
    text_features_tensors=torch.stack(text_features_list)
